...
## Decision
- The Inbox is a new repository, `surfingdog/inbox` (GitHub org `surfingdog`, to be created by
  Tiago), a pnpm monorepo: `apps/inbox` (Hono server + React SPA), `apps/site` (Astro + Starlight,
...
## Consequences
Two repos to release in step; the spec package is the contract between them. Local path
`~/surfingdog-inbox` until the org exists.
