...
A demo instance is running at [inbox.surfingdog.ai](https://inbox.surfingdog.ai/.well-known/agent-inbox.json). It belongs to Oficina Maré, a bicycle workshop in Ericeira that exists only in a seed file, and it answers agents right now: a manifest at `/.well-known/agent-inbox.json`, an OpenAPI document, a public MCP server, and an owner MCP server behind OAuth 2.1. You can list its services, check availability, request a booking and read it back with the access token you were given. If you send something malformed it will tell you exactly which field to fix.
