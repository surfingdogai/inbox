...
An instance can report to several networks, and each one lists it in its own directory. They are the `networks` setting, a map keyed by each network's https origin: `{"networks": {"https://network.surfingdog.ai": {"enabled": true}}}`. Settings are merged, never replaced, so adding one network leaves the others as they are, and switching one off is `"enabled": false` (it stays in the list, and anything it missed is sent when it is switched on again). At most eight. Each entry can also limit what that network gets, with `share: {"counts": false}` or `share: {"receipts": false}`; everything is on by default. A fresh instance lists `https://network.surfingdog.ai`, switched off; any directory that implements the same three endpoints works: `POST /v1/instances`, `POST /v1/instances/{domain}/ping` and `POST /v1/receipts`. For each network that is on, the instance registers its domain, which the network verifies by fetching the manifest and checking that `instance` is the https origin it was told (until it has, the instance asks again at most once a day); then it sends every hour its software version, its runtime and the number of bookings, orders, quotes and messages created in the last 24 hours, and it publishes every receipt it issues (below), which names the customer only by a pseudonym. No name, address or message content leaves the instance. Each network is called on its own, so one that is slow or down never holds up another, and Settings → Networks shows how each one is doing. An older document's `network: {url, join}` is read as one entry of this map. The numbers on the front page of surfingdog.ai are the sum of the pings `network.surfingdog.ai` receives.

## Receipts and two-sided reviews

When an item reaches a state that matters (`confirmed` for a booking, `paid` for an order) the instance issues a **receipt**: a compact JWS signed with the instance's Ed25519 key, which the manifest publishes in `receipt_keys` and `/.well-known/jwks.json` serves as a plain JWKS. The receipt is on the item for whoever may read it, and the customer's agent counter-signs it with `acknowledge_receipt`. Both sides then hold a small, verifiable proof that this transaction happened between these two parties, without either holding the other's address: the receipt names the customer by a pseudonym only this instance can produce. The full format and the rules are on the [Receipts](/docs/receipts/) page.
