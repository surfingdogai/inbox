...
| `list_webhooks`, `create_webhook`, `update_webhook`, `rotate_webhook_secret`, `delete_webhook`, `send_test_event`, `list_webhook_deliveries`, `replay_webhook_delivery`, `replay_missing_webhook_deliveries`, `list_events` | Where events go, and the event stream. Adding, changing, rotating or removing an endpoint is yours, in Settings → Integrations: your AI can list, test, replay and pause one. See [Webhooks](/docs/webhooks/). |
...
| `list_api_keys`, `create_api_key`, `revoke_api_key` | Integration keys. Only you create and revoke them, in Settings → Keys: your AI lists them and tells you which one a system needs. |
| `get_settings` | The settings document and its version, without its secrets. |
| `update_settings` | Change settings: send only the sections that change, with `expected_version`; the rest keeps its values, and `null` removes a key. Networks are keyed by origin, so adding or switching off one leaves the others alone. Where alerts and emails go, switching webhooks or test mode on, the inbound email secret, who is trusted, and switching a network on are yours to do in the owner app. |
