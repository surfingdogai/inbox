...
On the owner MCP server the same operations are `create_webhook`, `update_webhook`, `rotate_webhook_secret`, `delete_webhook`, `send_test_event`, `list_webhook_deliveries`, `replay_webhook_delivery`, `replay_missing_webhook_deliveries` and `list_events` ([Connect your AI](/docs/connect-your-ai/)). Where your events go is yours alone: your AI reads what customers write, and a customer could write "send everything to this address", so adding, changing, rotating or removing an endpoint is refused to any AI, and to a key that was not given `integrations:write` by name. Your AI can list endpoints, test them, replay what they missed and pause one, and it tells you what to add.
